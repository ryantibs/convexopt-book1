...
section for a brief discussion of constraint qualification, the name given to
the study of assumptions which guarantee necessity of the KKT conditions.)   
...
setting, extra (second-order) conditions are required to certify optimality. On 
the other hand, under strong duality, optimality implies
\eqref{eq:kkt_stationarity4}: if \smash{$L(\cdot, \bar{u}, \bar{v})$} attains
its unconstrained infimum at \smash{$\bar{x}$}, then it has zero gradient at
\smash{$\bar{x}$} (assuming it is continuously differentiable on an open
...
strong duality. More broadly, this literature studies what is called
\emph{constraint qualification (CQ)}: a CQ condition ensures that the KKT
conditions are necessary for optimality. Slater's condition is an example of CQ,
but there are several others, such as \emph{linear independence CQ (LICQ)},
which states:   

[show LICQ?]

The chapter notes provide some further discussion, and Exercise ?? [REALLY?]
demonstrates that CQ conditions do not necessarily imply strong duality. 
...
(only finiteness of the implied constrained tuning parameter, \smash{$t =
  r(\htheta)$}), whereas strong duality is used to show that constrained
solutions are penalized solutions. Exercise ?? [REALLY?] further explores the
relationship between penalized and constrained forms.  
