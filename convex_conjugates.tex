...
\item \parlab{xa:logistic_conjugate}
  For $f(x) = \sum_{i=1}^d \log(1+e^{-x_i})$, its conjugate is 
  \[
  f^*(u) = \sum_{i=1}^d \big( -u_i \log(-u_i) + (1+u_i) \log(1+u_i) \big), 
  \] 
  with $\dom(f^*) = [-1,0]^d$ (and where we interpret $0 \log(0) = 0$). 
...
smallest eigenvalues, respectively, of a symmetric matrix $A$. Then:
\begin{itemize}
