...
introduced in Exercise \ref{ex:moreau_proximal_identification}): 
...
help make this conclusion more salient, we will introduce one further concept: a
function that is both essentially smooth and essentially strictly convex is said
to be of \emph{Legendre type}. Using this nomenclature, Theorem
\ref{thm:conjugate_essentials} tells us that a closed convex function $f$ is of
Legendre type if and only if $f^*$ is. Theorem \ref{thm:conjugate_subgradients}
then says that for any such $f$, and $x \in \interior(\dom(f))$, we have $u =
\nabla f(x) \iff x = \nabla f^*(u)$. The next theorem summarizes.
...
The duality behind strong convexity and Lipschitz smoothness, as made precise in
Theorem \ref{thm:conjugate_smoothness}, is sometimes called the
\emph{Baillon-Haddad theorem}. Its proof is outlined in Exercise
\ref{ex:conjugate_smoothness}. We note that this theorem explains the symmetry 
...
If Moreau is the master of the proximal operator, then Rockafellar is the master
of conjugacy
...
Chapter 15 of Rockafellar extensive treatment of conjugate calculus
Chapter 26 of Rockafellar treatment of functions of Legendre type

% https://arxiv.org/pdf/0906.0807.pdf
...
\item Suppose that $f$ is strongly convex with parameter $m$. Fix any $u,v$ and
  ...
  \]
  Hint: argue that $F_u$ is strongly convex with parameter $m$ and is minimized
  at $x_u$, and then use Theorem \ref{thm:strong_convexity} property (iii).
...
\item Now suppose that $\nabla f^*$ is Lipschitz with parameter $1/m$. Again fix
  any $u,v$ and denote $x = \nabla f^*(u)$ and $y = \nabla f^*(v)$. Show that
  \[
  (x-y)^\T (u-v) \leq (1/m)\|u-v\|_2^2.
  \]
  Hint: use Theorem \ref{thm:lipschitz_smoothness} property (iv). NO!

\item Prove using the subgradient relation in Theorem
  \ref{thm:conjugate_subgradients} and property (iv) in Theorem
  \ref{thm:strong_convexity_nonsmooth} that $f$ is strongly convex with
  parameter $m$. NO!
\end{enumerate}
