...
saddle point, with the corresponding results being called \emph{minimax 
  theorems}. What distinguishes our study here from that in previous chapters is 
that the function $L$ is not restricted to be linear in $y$ (as all Lagrangian are).   
...
\end{equation}
we can directly observe $g(y) \leq L(x,y) \leq f(x)$, for any $x \in X$ and any
...
over $Y$, and \smash{$f(\bar{x}) = g(\bar{y})$}, for $f$ and $g$ defined as in  
...
\item Use Exercise \ref{ex:farkas_variations} part a to prove that there exists
