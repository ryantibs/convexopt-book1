...
\item Prove based on parts c and d that any finite number of sets $X(y_i)$, $i =
  1,\dots,n$ have a point in common.    

\index{Helly's theorem}
\item Complete the proof of Sion's minimax theorem by concluding that $X(y)$, $y
  \in Y$ have a point in common. Hint: you may use \emph{Helly's theorem}, which
  says that for a collection of convex and compact sets in $\R^d$, their
  intersection is nonempty provided that the same is true of any subcollection
  of at most $d+1$ sets.
\end{enumerate}
