...
\text{for $f(x) = \|x\|$, we have $f^*(u) = I_{\{y : \|y\|_* \leq 1\}}(u)$}.
\end{equation}
...
\end{enumerate}
  ...
  % \]
  % where recall $\langle Z, X \rangle = \tr(Z^\T X)$. Prove that $(\|X\|_p)_* =
...
\item \label{ex:dual_norm_dual3}
  Use \eqref{eq:dual_norm2_conjugate}, and the fact that $f^{**} = f$ for closed
  convex $f$, as in \eqref{eq:double_conjugate_reduction}, to verify $\|x\|_{**}
  = \|x\|$.  
