...
\eqref{eq:primal_problem} and $u^\star$ is a solution in the dual
\eqref{eq:dual_problem}. Then the inequalities in the last display, applied to
$x^\star$ and $u^\star$, must be all equalities:  
\begin{equation}
...
$u^\star$ is a dual solution, then this represents a set of prices for which
there is no advantage to violating the constraints versus respecting them.

\section{Duality gap}

emphasize that fstar = gstar implies solutions. state as a theorem?

then state converse of what's shown above. that saddle point of lagrangian
implies solutions.
