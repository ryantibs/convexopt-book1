...
\begin{equation}
\label{eq:lagrangian_bd2}
f^\star \geq \inf_{x \in C} \, L(x,u,v) \geq \inf_x \, L(x,u,v), \quad \text{for
  any $u,v$ such that $u \geq 0$}.  
\end{equation}

\begin{Remark}
The second inequality in \eqref{eq:lagrangian_bd2} is really the key idea behind
Lagrangian duality; had we stopped at the first inequality in
\eqref{eq:lagrangian_bd2}, we would have not have (yet) gotten anywhere
...
It is convenient to denote the right-most quantity in \eqref{eq:lagrangian_bd2}
by 
\index{dual function}
\begin{equation}
...
g(u,v) = \inf_x \, L(x,u,v), 
\end{equation}
which we call the \emph{Lagrange dual function}. Then \eqref{eq:lagrangian_bd2}
...
of the Lagrangian and the Lagrange dual function for a simple primal problem. 
Henceforth, our convention will be to drop the term ``Lagrange'' when referring
to the dual function \eqref{eq:dual_function} and dual problem
\eqref{eq:dual_problem}. 
...
\eqref{eq:dual_problem}. It follows from \eqref{eq:lagrangian_bd2} and the  
...
From \eqref{eq:lagrangian_bd1} and \eqref{eq:dual_function}, it follows that 
\begin{equation}
\label{eq:lagrangian_bd3}
f(x) \geq L(x,u) \geq g(u), \quad \text{for any primal feasible $x$ and dual
  feasible $u$}, 
\end{equation}
where by dual feasible we mean that $u \geq 0$.

\paragraph{Zero-sum game interpretation.}
