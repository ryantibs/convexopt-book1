...
to $\partial f^*(0)$. Furthermore, this is nonempty provided $0 \in
\relint(\dom(f^*))$ (Theorem \ref{thm:subgradient_existence}).      

More can be said: from our subdifferential theory (Theorem
\ref{thm:subgradient_boundedness}), the set of minimizers $\partial f^*(0)$ is
nonempty and bounded if and only if $0 \in \interior(\dom(f))$. Lastly, $f$ has
a unique minimizer if and only if $\partial f^*(0)$ is a singleton, which
happens if and only if $f^*$ is differentiable at the origin (Theorem
...
\begin{enumerate}[label=(\roman*)]
\label{thm:conjugate_recession} 
\item The set $S^\star$ of minimizers of $f$ satisfies $S^\star = \partial
  f^*(0)$, hence $f$ attains its infimum if and only $f^*$ is subdifferentiable
  at zero, which holds if $0 \in \relint(\dom(f^*))$.   
...
exceptions. Rockafellar's own take on the central importance of conjugacy, as is
explained in the preface to his book, was influenced by the unpublished lecture 
...
\item Prove that $0 \in \interior(\dom(f^*))$ if and only if $f$ has no
  ...
  \ref{thm:conjugate_recession} is similar to the first part of Theorem
...
\item Prove that $0 \in \relint(\dom(f^*))$ if and only if the only directions
  ...
  \ref{thm:conjugate_recession} is similar to the second part of Theorem
  \ref{thm:weierstrass_convex}.     
\end{enumerate}
\end{enumerate}
\end{xcb}
