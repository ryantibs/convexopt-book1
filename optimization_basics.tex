...
Equipped with a working knowledge of convex sets and functions, the core
...
\end{alignedat}
\end{equation}
where $f$, $g_i$, $i=1,\dots,m$ and $h_j$, $j=1,\dots,k$ are functions from  
...
\eqref{eq:optimization} where $f$, $g_i$, $i=1,\dots,m$ are all convex
functions, and $h_j$, $j=1,\dots,k$ are all affine functions. In other words,
the problem  
...
\end{alignedat}
\end{equation}
is convex whenever $f$, $g_i$, $i=1,\dots,m$ are convex (and $A,b$ are
arbitrary). 
...
& && h_j(x) = 0, \; j=1,\dots,k,
\end{alignat*}
Since $f$ is convex if and only if $-f$ is concave, the convex minimization 
problem \eqref{eq:convex_optimization} is thus also equivalent to a concave 
...
introduced above over to maximization problems). Likewise, we will broadly use 
the term ``convex optimization'', to refer to both convex minimization and
concave maximization.      

\begin{Remark}
In statistics, we do not typically denote the variable in an optimization
problem by $x$. We instead typically use $\beta$ or $\theta$, with $x$ often
being reserved for an input feature vector. We also rarely denote the solution
to an optimization problem by $\beta^\star$ or $\theta^\star$; we commonly use
...
common in these fields. This should not cause confusion, as the meaning (for
example, what is an optimization variable and what is a feature vector) should
...
\item Given class labels $y_i \in \{ -1, 1\}$ and feature vectors $x_i \in
  ...
  of the margin (the first term above is related to the inverse margin, see
  Exercise \ref{ex:svm_margin}), and violations to the margin condition (the
  second term is the sum of violation costs, where each violation is an instance
...
The proof of this fundamental result is elementary, and we break it up into two
steps. The first step is to check that for the convex problem
\eqref{eq:convex_optimization}, the set of feasible points forms a convex set;
this follows from convexity of the set $D$, convexity of the functions $f$ and 
...
\begin{Example}
The following are examples of the first-order optimality condition for convex
optimization.  
...
\item The Euclidean projection operator $P_C$ onto a convex set $C \subseteq 
  ...
  (z - x)^\T (y - z) \geq 0, \quad \text{for any $y \in C$}. 
  \end{equation}
  This is often referred to as the \emph{variational inequality} for a
  projection. It says that the vector pointing from $x$ to $z$ must have a
  positive inner product with the vector pointing from $z$ to $y$, for any $y
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.46\textwidth]{fig/variational_inequality.pdf}
\caption{Illustration of the variational inequality determining the optimality
  of the projection $z$ of a point $x$ onto a convex set. The angle between the
  line segments $xz$ and $yz$ must be at least a right angle (at least
  $90^\circ$), for any other point $y$ in the set.}   
...
= x^2$, which is convex. Thus, it can be useful to apply a monotone
transformation to the criterion in an optimization problem, as it can turn a
nonconvex problem into a convex one.  
...
contained in $\dom(f)$.)  We say a function $f$ is lower semicontinuous if it 
...
We say a function is \emph{closed} if its epigraph is closed, and we use this
...
sets of $f$ are closed (since $f$ is closed), this gives us the compactness
condition needed for Weierstrass' theorem.     
...
\end{equation}
where $\eta \in \R^k$ is called the \emph{natural parameter} of the exponential  
family (we use $\eta$, rather than $\theta$, to adhere to typical notation in
statistics). The function $\psi$ is called the \emph{log-partition function},
and we assume it to have open effective domain (in this case, the exponential 
family is called regular). 
% $T$ is the \emph{sufficient statistic}
% $h$ is the \emph{base density}

A remarkable fact: in any such exponential family distribution, the
log-partition function $\psi$ must be convex, by virtue of the fact that
$p_\eta$ must be a bona fide density and therefore must integrate to one (or sum
to one, in the discrete case); see Exercise \ref{ex:exponential_family_convex}. 
This means that the map $\eta \mapsto p_\eta(z)$ is always log-concave (for
fixed $z$), and the resulting maximum likelihood problem
\[
...
core problem form in modern statistics. 
...
\ref{thm:weierstrass_convex}. Later, in Chapter \ref{sec:duality_minima}, we
will derive a generalization via duality arguments (this will also provide
simple proofs for the logistic and Poisson results).        
...
\item In Poisson regression \eqref{eq:poisson_regression}, where $\cY = \{0, 1,
  2, \dots\}$, a solution exists provided:  
  \begin{equation}
...
The lasso was first proposed by \cite{tibshirani1996regression}, and similar ideas
appeared independently in \cite{chen1998atomic}. The support vector machine
...
lasso and the SVM (including its kernel version) are of course cornerstone
methods in statistics and machine learning, and there are many excellent books
that cover them in great detail, including \cite{hastie2009elements,
  scholkopf2002learning, hastie2015statistical}. Later, in Chapter,
\ref{chap:lasso}, we will dive into various properties of the lasso in more
detail.
...
\item \label{ex:svm_margin} 
  ...
  \] 
  is equal to $2/\|\beta\|_2$. Hint: pick two points $x_+ \in H_+$ and $x_- \in
  H_-$, and argue that the margin is the inner product of $x_+-x_-$ with the
  outward normal $\beta/\|\beta\|_2$ pointing from $H_-$ to $H_+$.  
...
\item Show that the set of minima $S^\star$ of $f$ is nonempty and compact,
  ...
  sequence \smash{$\{t_k\}_{k=1}^\infty$} with $t_k \to f^\star = \inf_x \,
  f(x)$ as $k \to \infty$, we have \smash{$S^\star = \cap_{k=1}^\infty \{x :
    f(x) \leq t_k\}$}.   
...
\item \label{ex:direction_recession}
  We work through the proof of Lemma \ref{lem:direction_recession}. Let us
  introduce the concept of the \emph{recession cone} of a convex set $C
  \subseteq \R^d$, denoted $R_C$: this is the set of vectors that are directions
  of recession of $C$. It is straightforward to check that $R_C$ is indeed a
...
\item Let $C_1 \supseteq C_2 \supseteq C_3 \supseteq \cdots$ be a sequence of
  nonempty nested closed convex sets with the following two properties: (i)
  \smash{$R_{C_k} = L_{C_k}$} for each $k$, and (ii) \smash{$L_{C_k} = L$} for 
  each $k$, for each set in the sequence, the only directions of recession are
  directions of linearity, where all sets share a common lineality space. Prove  
  that such a sequence must have a nonempty intersection, and furthermore,      
  \[
...
\index{exponential family}
\item \label{ex:exponential_family_convex} 
  We will prove that for a regular exponential family distribution, with
...
\begin{enumerate}[label=\alph*.]
\item Let \smash{$\Lambda_0(t) = \int_0^t \lambda_0(t) \, \d{t}$} be the
  base cumulative hazard function. Under the Cox model \eqref{eq:cox_model},
  show that the density of the survival time $t_i$ for subject $i$  
  is 
  \[
...
\item Repeat the profile likelihood calculation in the case of right censoring:
  ...
  \smallskip
  An important note: now we must restrict the cumulative baseline hazard
  function $\Lambda_0$ to be piecewise constant with jumps at the failure times
  (the subset of $y_i$, $i=1,\dots,n$ such that $\delta_i = 1$) \emph{by
    assumption}. Under censoring, this no longer falls out of maximizing the
  full likelihood.               
\end{enumerate} 
\end{enumerate}
\end{xcb}