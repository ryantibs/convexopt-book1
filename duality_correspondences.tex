...
\caption{A cone $K$ and its dual cone $K^*$, defined by the normal vector to all
  halfspaces containing $K$.}  
\label{fig:dual_cone}
\end{figure}
...
Recall (as briefly in Chapter \ref{sec:polyhedra}), a polyhedron is an
intersection of halfspaces, which we can write as $P = \{x : Ax \leq b\}$ for 
a matrix-vector pair $A,b$ of compatible dimensions. The \emph{dual polyhedron} 
to $P$ is defined as  
...
it clear that $P^*$ is itself polyhedral in the case that $P$ is bounded. The 
unbounded case is discussed in Exercise \ref{ex:dual_polyhedron}.  
 
Let us assume henceforth that $P$ is bounded polyhedron, which recall we call a
polytope, with $0 \in \interior(K)$. While Theorem \ref{thm:hv_representation},
the main representation theorem for polytopes, can be established directly using 
algebraic arguments and Carath\'{e}odory's theorem (Theorem
\ref{thm:caratheodory}), we demonstrate in this section how it can be
alternatively understood using polyhedral duality.   

To this end, suppose that we have the H-representation $P = \{ x : Ax \leq
b\}$. Then by definition, 
\begin{align}
\nonumber
P^* &= \{ x : y^\T x \leq 1, \; \text{for all $x$ with $Ax \leq b$} \} \\
\label{eq:v_representation_dual}
&= \bigg\{ \sum_{i=1}^m t_i \frac{a_i}{b_i} : \text{$t_i \geq 0$, for
  $i=1,\dots,m$, and $\sum_{i=1}^m t_i = 1$} \bigg\}.
\end{align}
The second step above can be verified using Farkas' lemma (Exercise 11.20),
where $a_1,\dots,a_m \in \R^d$ denote the rows of $A$, and $b_1,\dots,b_m > 0$ 
(as $0 \in \interior(P)$). Defining $y_i = a_i/b_i$, $i = 1,\dots,m$, we have
thus produced the dual V-representation $P^* = \conv\{y_1,\dots,y_m\}$.      

Suppose instead that we have the V-representation $P =
\conv\{x_1,\dots,x_n\}$. Again by definition,  
\begin{align}
\nonumber
P^* &= \big\{ x : y^\T x \leq 1, \; x \in \conv\{x_1,\dots,x_n\} \big\} \\
\label{eq:h_representation_dual}
&= \{ y : y^\T x_i \leq 1, \; i = 1,\dots,n \},
\end{align}
the second step holding by convexity (Exercise 11.21). In other words, we have
produced the dual H-representation $P^* = \{ y : Xy \leq 1\}$, where $X$ is the 
matrix with rows $x_1,\dots,x_n \in \R^d$.   

Thus we have established, in a sense made precise above, that halfspaces in an  
H-representation of $P$ correspond to vertices in a V-representation of $P^*$,
whereas vertices in a V-representation of $P$ correspond to halfspaces in an
H-representation of $P^*$. In fact, more is true---there is a one-to-one 
correspondence between all faces of $P$ and faces of $P^*$.  

\begin{Theorem}
face theorem
\end{Theorem}

This theorem, which can be verified using direct arguments (Exercise 11.22),
gives us a way to understand Theorem \ref{thm:hv_representation}.
...
\cite{bental2023convex} (Chapters 1.2 and 3.2). Exercise
\ref{ex:dual_polyhedron} is based on \cite{rockafellar1970convex} (Chapter 19). 
...
\item \label{ex:dual_polyhedron}
  ...
  generalization of Theorem \ref{thm:hv_representation} is as follows:      
  \begin{equation}
