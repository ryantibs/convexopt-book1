...
\item Let $f$ be closed and convex, and consider again its Moreau envelope
  $f_\lambda$ and associated proximal operator $\prox_{\lambda f}$ for
  $\lambda>0$. Fix any $x$.     
...
\item For essentially strictly convex $f$, show that $\prox_{\lambda  f}(x) =
  \nabla f^*(\nabla f_\lambda(x)) = (\nabla f)^{-1} (\nabla f_\lambda(x))$. 
  Hint: combine part a with Theorems \ref{thm:conjugate_essentials} and
