...
\item Given $A \in \R^{k \times d}$, $b \in \R^k$, $g \in \R^d$, and 
  $h \in \R$, where $b \geq 0$, consider two statements: 
  \begin{itemize}
...
\item The other direction---that infeasibility of problem
  ...
  \end{itemize}
  We will maintain $m,k$ for the dimensions of $u,v$, respectively. Now with
  this notation in place, define the sets    
  \begin{align*}
...
\item For an SDP \eqref{eq:sdp_primal} that satisfies condition (i) in Corollary 
  ...
  \eqref{eq:sdp_primal} is feasible, in which case a dual solution
  exists. Hint: apply the general Slater condition from Theorem
  \ref{thm:slater_conic}.  Note: this result can be compared to what is known 
  for LPs, from Exercise \ref{ex:lp_slater} part c, the conclusion there being 
  that \emph{both} primal and dual solutions exist.
...
\item Similarly, for an SDP whose dual \eqref{eq:sdp_dual} satisfies condition
  ...
  \ref{thm:slater_conic}, now on the dual. Note: this result can be compared
  to what is known for LPs, from Exercise \ref{ex:lp_slater} part d, the
  conclusion there being that \emph{both} primal and dual solutions exist.  
