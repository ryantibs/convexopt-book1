...
f( \E[X] ) \leq \E[ f(X) ],
\end{equation}
(provided these expectations exist). Oppositely, if $f$ is concave then
$f( \E[X] ) \geq \E[ f(X) ]$ (provided such expectations exist). These
inequalities can be seen as generalizations of the defining properties of
convexity and concavity, \eqref{eq:convex_function} and
\eqref{eq:concave_function}, respectively, from discrete to arbitrary
distributions.
...
$P(C)$ is convex. Also, if $D \subseteq \R^d$ is convex then the perspective
...
the linear-fractional image $f(C)$ is convex. Also, if $D \subseteq \R^k$ is 
...
differentiable convex $f$, statements (i)--(v) are all equivalent. 
\end{Theorem}
...
\item Show that $\text{(i)} \implies \text{(iv)}$ by the Cauchy-Schwartz
  inequality, proving the first display in Theorem
  \ref{thm:lipschitz_smoothness}.  

\index{Lipschitz smoothness} 
\item When $f$ is twice differentiable, show that $\text{(v)} \implies
  \text{(iii)}$ by studying a first-order Taylor expansion of $f$ with
...
\item Show that $\text{(iv)} \implies \text{(i)}$ by the Cauchy-Schwartz
  inequality, proving the first display in Theorem \ref{thm:strong_convexity}. 
...
\item For convex $f$, prove the final part of Theorem
  ...
  \nabla f(x)\|_2^2. 
  \]
  Use Cauchy-Schwartz to conclude that $f$ is Lipschitz smooth, as desired.
