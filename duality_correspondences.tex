...
\begin{equation}
\label{eq:polar_set}
C^\circ = \{ y : y^\T x \leq 1, \; x \in P \}.
\end{equation}
Notice that for a polyhedron $P$, its polar set is just another term for its
dual polyhedron in \eqref{eq:dual_polyhedron}, $P^\circ = P^*$. Meanwhile, for a
cone $K$, its is polar set is the negative dual cone in \eqref{eq:dual_cone},
$K^\circ = -K^*$. To see this, note ... 

\begin{equation}
\label{eq:polar_set_polar}
C^{\circ\circ} = C.
\end{equation}
% Indeed, it is immediate from the definitions that K c K**. On the
% other hand, if X o ¢ K there exists a hyperplane H= {z E Rd I<z,Yo)= I}
% strictly separating X o and K, i.e. such that ( x, Yo) < 1 for all x E K and
% <xo, Yo) > 1. Therefore Yo E K* and then <xo, Yo) > 1 shows that
% xo ¢ K** .
  ...
  T_C(x) = \{ y : x + ty \in C, \; \text{for small enough $t>0$} \},
  \end{equation}
...
\item \label{ex:face_duality}
  This exercise outlines the proof of Theorem \ref{thm:face_duality}. Let $P$
  be a polytope with $0 \in \interior(P)$, $P^*$ be its dual, and $\Psi$ be the
  map defined in the theorem. It is trivial to check that $\Psi(\emptyset) =
  P^*$ and $\Psi(P) = \emptyset$, so we may restrict our attention to a proper
  face $F \in \cF(P)$.  
