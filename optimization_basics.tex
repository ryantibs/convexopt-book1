...
Below we describe several important properies of convex optimization problems,
...
formulation \eqref{eq:characteristic_formulation} of a general optimization
...
$p_\eta$ must be a bonafide density and therefore must integrate to one (sum to
...
\ref{sec:conjugates_dual_problems}, we will give a generalization via duality
arguments (this also provides simple proofs for the logistic and Poisson
results).        
...
\cite{scholkopf2002learning}, and \cite{hastie2015statistical}.
...
\item \label{ex:saddle_point} 
  ...
  \minimize_x \quad \sup_{u \geq 0, \, v} \, \underbrace{f(x) + \sum_{i=1}^m u_i 
    g_i(x) + \sum_{j=1}^k v_j h_j(x)}_{L(x,u,v)}. 
  \end{equation}
