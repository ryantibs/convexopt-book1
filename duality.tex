...
g(u) = \min_{j=1,2,3} \, \bigg\{ \frac{1}{50} R_j^4(u) - R_j^2(u) + 2 R_j(u) +
25 - u R_j(u) - 4u \bigg\}, 
\end{equation}
where 
\begin{multline*}
...
Then strong duality \eqref{eq:strong_duality} holds. That is, to be more
explicit: the optimal criterion values $f^\star$ in \eqref{eq:primal_problem}
and $g^\star$ in the corresponding dual problem \eqref{eq:dual_problem} satisfy 
$f^\star = g^\star$. 
\end{Theorem}
...
application of the separating hyperplane theorem, and is outlined in Exercise
...
duality holds if \eqref{eq:lp_primal} is feasible. However, this is not all: one
can check (Exercise \ref{ex:lp_dual_dual}) that the dual of the dual problem
\eqref{eq:lp_dual} is in fact the primal problem \eqref{eq:lp_primal}.
Therefore, strong duality in problems \eqref{eq:lp_dual} and
\eqref{eq:lp_primal} mean the same thing, and applying Slater's condition to
\eqref{eq:lp_dual} says that strong duality again holds if \eqref{eq:lp_dual} is
feasible. Next, we summarize these conclusions.

\index{strong duality!linear program}
\begin{Corollary}
For an LP, strong duality holds if the primal or the dual problem is 
feasible. 
\end{Corollary}

Still, more can be said, by characterizing what happens when the primal or the
dual is infeasible; see Exercise \ref{ex:lp_infeasible}. Exercises
\ref{ex:lasso_dual} and \ref{ex:svm_dual} explore the lasso and SVM problems
(both QPs), their duals, and the implications of Slater's condition.  
...
credit BV for inspiration of nonconvex quartic example

credit BV for interpretations

Bertsekas: nonlinear farkas lemma? is it the same as:
Nemirovksi: convex theorem of alternatives. inspiration for exercises below
...
term involving $x^2$. Show that Cardano's formula, for the roots of a depressed
cubic, yields the three roots $x = R_j(u)$, $j = 1,2,3,\dots$ as defined in the  
example. Then argue that the dual function $g$ in
\eqref{eq:nonconvex_quartic_dual} can be obtained by taking the minimum value of
the Lagrangian over these roots. 
...
\item \label{ex:lp_infeasible}

\item \label{ex:lasso_dual}
We explore duality for the lasso problem,      
...
for a response vector $y \in \R^n$ and feature matrix $X \in \R^{n \times
  d}$. Prove that the dual of \eqref{eq:lasso_primal} is (equivalent to)
\begin{equation}
\label{eq:lasso_dual}
...
condition automatically holds, and thus strong duality always holds in between 
...
We explore duality for the SVM problem,      
...
\item convex theorems of alternatives?
\end{enumerate}
\end{xcb}
