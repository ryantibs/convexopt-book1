...
\item \label{ex:fritz_john_condition} 
  We now use Gordan's lemma to prove necessity of what is known as the
  \emph{Fritz John condition} \cite{john1948extremum} for optimality in the
  problem 
  \begin{equation}
...
\item Prove that there exists \smash{$\lambda_0 \geq 0, \lambda_i \geq 0$, $i
  ...
  h_i(\bar{x}).
  \end{equation}
  Hint: use Gordan's lemma, with $y = -z$.
\end{enumerate}
...
\item \label{ex:subgradients_kkt}
  ...
  notational ease (as we did previously in studying Slater's condition in
  Exercise \ref{ex:convex_theorem_alternatives}), we will:   
  \begin{itemize}  
...
\item Let $a_j \in \R^d$, $j = 1,\dots,k$ be the rows of $A$. Show that there
  ...
  \leq b$), show that the normal cone to $C$ at \smash{$\bar{x}$} is
  \smash{$\cN_C(\bar{x}) = \{ A^\T \mu : \mu \geq 0, \, \text{$\mu_j = 0$ for
      $a_j^\T \bar{x} < b_j$} \}$}.     
...
\item To finish, apply the subgradient rule for a maximum to $h_0$, and relabel
  ...
  0$ for \smash{$h_i(\bar{x}) < 0$} and $v_j = 0$ for \smash{$a_j^\T \bar{x} < 
    v_j$}, such that 
  \[
