...
We now discuss some basic properties and interpretations of proximal mappings. 
...
\item \parlab{xa:trace_norm_proximal_mapping} 
  ...
  elements $a$. The operator $M_\lambda = \prox_{\lambda f}$ here is called 
  \emph{matrix soft- thresholding}. % RJT: spacing fix
...
\item How do we know that the $\ell_1$ penalty in the lasso problem
  ...
  \]
  where $S_{\lambda t}$ is the soft-tresholding operator at the level $\lambda
  t$, as in \eqref{eq:soft_thresholding}. This certifies that the lasso problem
  admits a sparse solution (as the output of $S_{\lambda t}$ is directly
  sparse), with generally a greater degree of sparsity for a larger $\lambda$.    
...
\item How do we know that the trace norm penalty in the matrix completion
  ...
  \]
  where $M_{\lambda t}$ denotes matrix soft-thresholding at the level $\lambda
  ...
  $M_{\lambda t}$ is directly low-rank), with generally a smaller rank for a
  larger $\lambda$.    
\end{enumerate}
\end{Example}

The idea demonstrated in the last example carries over even beyond the case of a
...
\index{positive semidefinite cone!projection operator}
...
\index{probability simplex!projection operator}
...
\index{isotonic cone}
\index{isotonic cone!projection operator}
...
Next, we describe two key properties of projection maps. Both of these
...
is nonzero. This proves the firm nonexpansiveness of $\prox_f$, as desired. 
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.65\textwidth]{fig/moreau_envelope.pdf}
...
minimization of $f$ and of $f_\lambda$ are equivalent problems. Fifth, as a 
property that is tied to the fourth one  (Exercise \ref{ex:moreau_envelope} part 
b), the point $z = \prox_{\lambda f}(x)$ achieves the infimum in
...
\end{equation}
Fifth, and perhaps most importantly, the Moreau envelope $f_\lambda$ is
...
Moreau envelopes, then they must be the same function.  This is a fundamental
property. Furthermore, the relationship between the proximal operator and the
gradient of the Moreau envelope \eqref{eq:moreau_envelope_gradient} then
transfers this identifiability property to the former (up to an additive
constant). We state these results next, to conclude the chapter.   
...
\item \label{ex:matrix_norm_proximal_mapping1}
  ...
  X - Z \in \lambda \partial f(Z),
  \end{equation}
  which is of course just the appropriate translation of the subgradient 
  characterization \eqref{eq:proximal_subgradient_characterization} to
  matrices. 
...
\item Show using the Cauchy-Schwartz inequality that
  ...
  Cauchy-Schwartz inequality that \eqref{eq:projection_firm_nonexpansiveness1}
...
\item Assume that $f \infconv h = g \infconv h$ for a closed, convex, and
  ...
  \[
  \inf_x \big( f_\lambda(x) - v^\T x \big) = \inf_x \big( g_\lambda(x) - v^\T x
  \big), \quad \text{for all $v$}.
  \]
