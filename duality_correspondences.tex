...
Directly from \eqref{eq:norm_dual}, we may infer that $x^\T z \leq \|x\|$ for
...
Dual of dual norm is original. $\|x\|_{**} \leq \|x\|$ by Holder. Other way
around: $L=\{ax : a \in \R\}$, and let $f(ax) = a\|x\|$ be a map on $L$. Check
$f$ is bounded, linear, and has operator norm 1. By HB we can extend this to all
of $\R^d$, as $F$, while preserving its norm. This means $\|x\|_{**}
\sup_{\|z\|_* \leq 1} \, z^\T x \geq F(x) = \|x\|$. 
