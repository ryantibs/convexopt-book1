...
$x^\star,  u^\star, v^\star$) simply says 
...
are penalized solutions. Exercise ?? further explores this relationship.
...
hold. Exercises ??-?? provide further exploration.
...
authors write the KKT stationarity condition in different and generally
...
Constraint qualification itself carries a large literature. Cite reviews. It
seems rare that it is treated in tandem with typical convex analysis. An
exception in BL 2006. Who give a nice holistic treatment. Exercises are based on
theirs.

KKT conditions are used extensively in the analysis of statistical
estimators. Exercise blah part b based on \cite{witten2011new,
  mazumder2012exact}.
...
\item Let \smash{$\hbeta^{\textnormal{lasso}}$} be a lasso solution in 
  \eqref{eq:lasso_primal2} and \smash{$\hbeta^{\textnormal{Dantzig}}$} be a
  Dantzig selector solution in \eqref{eq:dantzig_selector}, for the same
  $X,y,\lambda$. Prove that \smash{$\|\hbeta^{\textnormal{Dantzig}}\|_1 \leq
...
\item Something on constrained vs penalized???
...
\item Let \smash{$S = \frac{1}{n} \sum_{i=1}^n (x_i - \bar{x}) (x_i -
  ...
  produces a sparse estimate of the inverse covariance matrix $\Sigma^{-1}$,
  defined by solving    
  ...
  \|\Theta\|_1, 
  \end{equation}
  where $\lambda \geq 0$ is a tuning parameter, and \smash{$\|\Theta\|_1 =
    \sum_{i,j=1}^d |\Theta_{ij}|$} is the elementwise $\ell_1$ norm of
  $\Theta$. The above criterion is an $\ell_1$-penalized negative log
  likelihood, where $-\log(\det(\Theta)) + \tr(S\Theta)$ is the negative log
  ...
  \eqref{eq:graphical_lasso} is strictly convex (note that log-det is a strictly
  concave function, recalling part a of the last exercise).      
...
\item Define $S_\lambda$ by elementwise soft-thresholding
  ...
  1,\dots,d. 
  \]
  Suppose that $S_\lambda$ is block-diagonal with blocks $B_1,\dots,B_m$; that  
  is, the sets $B_1,\dots,B_m$ form a partition of $\{1,\dots,d\}$, and
  $(S_\lambda)_{ij} = 0$ whenever $i \in B_k$ and $j \in B_\ell$, with $k \not= 
  \ell$. Prove that the graphical lasso  solution \smash{$\hat\Theta$} in
  \eqref{eq:graphical_lasso} must have the same block-diagonal structure as
  $S_\lambda$. Hint: use \eqref{eq:graphical_lasso_optimality}.    
\end{enumerate}

\item Use subgradient sum property, under slater, to derive KKT conditions from
  subgradient optimality?

\item Something on perbutation/sensitivity analysis?

\item Gordan's theorem. Prove from separation, or from Farkas? 

\item State Fritz John condition and cite it. Prove it. By Gordan's. 
% Theorem 2.3.6 of BL 

\item State MFCQ and cite it. Prove it. By FJ and Gordan's implying $u_0 \not=
  0$. 
% Theorem 2.3.8 of BL 

\item Show that LICQ implies MCFQ. By Gordan's (once more). 
