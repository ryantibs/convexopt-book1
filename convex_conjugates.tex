...
and duality, the topic of the current chapter is at the same time simple and yet
powerful.       
...
\parlab{par:conjugate_translation}
...
relative interiors with a point in common, it holds that $(f + g)^* = f^*
...
\section{Conjugates and minima*}
\label{sec:conjugate_minima}

Conjugacy provides a lens through which we can interpret infima and minima. To
explore this, we can begin by simply observing that $f^*(0) = \sup_x \, - f(x)$,
or 
\[
-f^*(0) = \inf_x \, f(x),
\]
that is, the conjugate $f^*$ at zero equals the infimum of $f$, and this holds
whether of not the infimum is attained. Toward understanding attainment, for
closed convex $f$ with nonempty domain we know that $x \in \partial f(y) \iff y
\in \partial f^*(x)$ (Theorem \ref{thm:conjugate_subgradients}). In particular,
choosing $y = 0$ reveals the following equivalence: $f$ has a minimizer at $x$ if
and only if and only if $x \in \partial f^*(0)$. In other words, the set of
minimizers of $f$ equals $\partial f^*(0)$, and therefore, this set is nonempty 
provided $0 \in \relint(\dom(f^*))$ (Theorem \ref{thm:subgradient_existence}).    
...
we must first define some concepts. A function $f : \R^\d \to (-\infty,\infty]$
is said to be \emph{essentially smooth} if the following holds, for $C =
\interior(\dom(f))$:     
\begin{enumerate}[label=(\roman*)]
\item $C \not= \emptyset$;
...
\emph{essentially strictly convex} if it is strictly convex on all convex
subsets $S$ of $ \{x : \partial f(x) \not= \emptyset\}$. (Strict convexity on
$S$ means that \eqref{eq:strictly_convex_function} holds restricted to $x,y \in
S$.) An important special case is a function that is finite and strictly convex
on all of $\R^d$.   
...
which we describe in this section to conclude the chapter.
...
\item Prove the conjugate sum rules in Property
  ...
  % For example, Theorem 16.4 in Rockafellar (1970)

\item Prove that if $C,D$ are nonempty convex sets whose relative interiors have
  a point in common, then the support function of set sum $C+D = \{ c + d : c 
  \in C, \, d \in D \}$ satisfies $h_{C+D} = h_C + h_D$. Hint: use Property
  \parref{par:conjugate_general_sum}. 
  % For example, Corollary 16.4.1 in Rockafellar (1970)

\index{optimization problem!existence of solutions}
\index{direction of recession}
\item \label{ex:conjugate_recession}
  Let $f$ be closed and convex. We study the relation between directions of
  recession of $f$ \eqref{eq:direction_recession} and the effective domain of
  its conjugate $f^*$.      
...
% convexity and the fact the subgradient is empty outside $\interior(dom(f))$.
