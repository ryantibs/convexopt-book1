...
\item Given a response vector $y \in \R^n$ and feature matrix $X \in \R^{n
  ...
  $\lambda \to 0$. 
  % TODO: should we refer to parameterized convex problems section?
...
\begin{Example}
Demonstrating the generality of cone programs, the following two examples show
how arguably the most common GLM optimization problems outside of linear
regression \eqref{eq:linear_regression} (itself a QP) are cone programs. See
Exercise \ref{ex:geometric_program} for details.
...
\index{linear matrix inequailty}
