...
\item In this exercise, we will examine the dual of the Dantzig selector
  problem \eqref{eq:dantzig_selector}, and interpret the associated primal-dual
  relationship.   

\begin{enumerate}[label=\alph*.]
\item Show that the dual of \eqref{eq:dantzig_selector} is (equivalent to) 
\index{Dantzig selector!dual problem}
  ...
  w\|_\infty \leq 1.
  \end{equation}
  Hint: reparametrize the constraint in \eqref{eq:dantzig_selector} as $-\lambda
  1 \leq X^\T (y - X\beta) \leq \lambda 1$.
...
\item Show that the primal-dual relationship is
  ...
  \eqref{eq:dantzig_selector}, \eqref{eq:dantzig_selector_dual}, respectively. 

\item Argue that we may assume without a loss of generality that $y \in
  \col(X)$, and therefore write \smash{$y = X \hbeta^{\textnormal{ls}}$} for a 
  (possibly nonunique) least squares solution
  \smash{$\hbeta^{\textnormal{ls}}$}, and write the
  dual problem \eqref{eq:dantzig_selector_dual} as
  \begin{equation}
  ...
  coefficients whose signs agree with those of least squares coefficients (first
