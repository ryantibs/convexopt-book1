...
\item Given $A \in \R^{k \times d}$, $b \in \R^k$, $g \in \R^d$, and $h \in \R$,
  ...
  % Alternative proof via separating hyperplanes: there exists a vector $s \geq
  % 0$ and scalar $t \geq 0$ such that      
  % \[
  % s^\T (Ax - b) \geq t (g^\T x - h), \quad \text{for all $x \in \R^d$}.
  % \]
  % To show this, apply the separating hyperplane theorem, Theorem
  % \ref{thm:separating_hyperplane}, to 
  % \[
  ...
  % \bigg\}. 
  % \]
  % Then finally argue that we must have $t > 0$ (otherwise this $s^\T (Ax - b) 
  % \geq 0$ for all $x$ which implies $s = 0$, a contradiction to the nonzero
  % normal vector given by the supporting hyperplane theorem).
\end{enumerate}
...
\item \label{ex:lp_sdp_differences}
  ...
  \item for an LP, a solution exists whenever its optimal criterion value is
    ...
    share a ``proper'' direction of strict recession, in which the criterion is   
...
\begin{enumerate}[label=\alph*.]
\item For an SDP that satisfies the general conic Slater condition in Theorem
  \ref{thm:slater_conic}, show (as a consequence of this theorem) that $f^\star
  > -\infty$ if and only if the dual is feasible, in which case a dual solution 
  exists. Note: this can be compared to what is known for LPs, from Exercise     
  \ref{ex:lp_slater} part c, where the conclusion is that \emph{both} primal and 
  dual solutions exist.

\item Similarly, for an SDP whose dual problem satisfies the Slater condition in
  Theorem \ref{thm:slater_conic}, show that $g^\star < \infty$ if and only if
  the primal is feasible, in which case a primal solution exists. Note: this can 
  be compared to what is known for LPs, from Exercise \ref{ex:lp_slater} part d,  
  where the conclusion is that \emph{both} primal and dual solutions exist.  

\item Show that the statements above cannot be sharpened for SDPs in general, by
 ...
 \end{bmatrix} \succeq 0.  
  ...
  equals the primal optimal value.
...
\item Show that for an SDP, the duality gap $f^\star - g^\star$ can be positive
 ...
 \end{bmatrix}.
  ...
  dual optimal value, and show that the duality gap is $\alpha$. Explain why
  this example does not violate Theorem \ref{thm:slater_conic}.  Note: this
  example is taken from from \cite{ramana1997exact}, who credit
