...
since this will help us reason about particular problems and their properties in 
...
optimization problem which takes the form
...
\item Under the same setup as in the last example, the \emph{Dantzig selector} 
  ...
  \begin{alignedat}{2}
  &\minimize_\beta \quad && \|\beta\|_1 \\
  &\st \quad && \|X^\T (y - X\beta) \|_\infty \leq \lambda, 
  \end{alignedat}
  \end{equation}
...
similar equivalence exists for operator norm constraints (Exercise
...
intricate to prove; we return to them later through the lens of duality in
semidefinite programs, in Exercise \ref{ex:trace_norm_semidefinite}.
...
\item Now for the constrained analogs, by similar arguments, prove that   
  \begin{alignat*}{2}
  &\minimize_X \quad && f(X) \\
  &\st \quad && \|X\|_{\op} \leq s \\
  & && x \in C
  \end{alignat*}
  and
  \begin{alignat*}{2}
  &\minimize_X \quad && f(X) \\
  &\st \quad && \begin{bmatrix} s I & X \\ 
...
\item A generalization of a QP is a \emph{quadratically-constrained quadratic
  ...
  &\st \quad && \frac{1}{2} x^\T Q_i x + c_i^\T x + b_i \leq 0, \;
  i=1,\dots,m \\     
  & && Gx = h,
  \end{alignedat}
  \end{equation}
...
\item \label{ex:geometric_program}
  ...
  &\st \quad && \sum_{k=1}^{K_i} \exp(a_{ik}^\T x + b_{ik}) \leq 1, \;
  i=1,\dots,m \\   
  & && Gx = h, 
  \end{alignedat}
  \end{equation}
...
\item Prove that \eqref{eq:geometric_program} can be rewritten as 
  ...
  &\st \quad && c_i^\T y_i \leq 1, i=1,\dots,m \\
