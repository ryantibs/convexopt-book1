...
the set of minimizers of $F$ over its entire effective domain. We will also
abide by the convention that the $\argmin$ operator returns the minimizer itself
when the latter is unique (and not the set containing the minimizer).       
...
: x \in (I + \lambda \partial f)(z) \}$. Inspecting the last display, we can see
that it reveals something interesting: the preimage appearing in the second line  
must actually be \emph{single-valued}, because there is exactly one point
\smash{$z = \prox_{\lambda f}(x)$} which satisfies $z \in (I + \lambda \partial
f)^{-1}(x)$. This allows us to write  
...
\item \parlab{xa:trace_norm_proximal_mapping} 
  ...
  elements $a$. The proximal operator here is often denoted $m_\lambda$, and
...
\item Let $y_i \in \R$, $i = 1,\dots,n$ be independent draws from an
  ...
  differentiable convex $\psi$ (recall that this is always true in any regular
  exponential family distribution). First note that the proximal map decomposes
  across coordinates: for $i=1,\dots,n$,      
  \[
...
\item \parlab{par:slope_proximal_mapping}
  ...
  \[
  \prox_f(x) = \argmin_z \, \bigg\{ \frac{1}{2} \|x - z\|_2^2 + \sum_{i=1}^d
  \lambda_i |z|_{(i)} \bigg\} 
  \]
...
\end{equation}
Finally, denoting by \smash{$|\cdot|_{\TV}$} the TV seminorm in \eqref{eq:tv},
for any permutation invariant $f$ and $\lambda > 0$, it holds that (Exercise
\ref{ex:generalized_tv_proximal_mapping}):   
\begin{equation}
\label{eq:generalized_tv_proximal_mapping}
\prox_{f + \lambda |\cdot|_{\TV}} = \prox_f \circ \prox_{\lambda |\cdot|_{\TV}}.    
\end{equation}
...
The examples below show how we can use the closed-form of proximal maps (when
...
\item How do we know that the $\ell_1$ penalty in the lasso problem
  \eqref{eq:lasso} induces sparsity in its solution(s)? Using the proximal
  ...
  with generally a greater degree of sparsity for larger $\lambda$.     
...
\item How do we know that the trace norm penalty in the matrix completion
  ...
  $m_{\lambda t}$ is low-rank), with generally a smaller rank for larger
  $\lambda$.       
\end{enumerate}
\end{Example}
...
\item For $C = \{x : a^\T x = b\}$ with $a \not= 0$, a hyperplane, we have
  $P_C(x) = x + (b - a^\T x) a / \|a\|_2^2$; whereas for $C = \{x : a^\T x \leq 
...
\item For $C = \{x : \one^\T x  = 1, \, x \geq 0 \}$, a special polyhedron
  called the \emph{probability simplex}, the projection $P_C(x)$ can be computed
  efficiently with a specialized algorithm whose cost is dominated by sorting
  the entries of $x$ (and is thus nearly linear-time); see Exercise 
...
(x - y)^\T \big( P_C(x) - P_C(y) \big) \geq \| P_C(x) - P_C(y) \|_2^2, \quad 
\text{for all $x,y$}. 
\end{equation}
...
nonexpansive- ness % Note: spacing fix
...
nonexpansiveness is equivalent to the property:     
\begin{equation}
...
\caption{Illustration of Euclidean projection operators onto convex and
  nonconvex sets, $C$ and $D$. The projections of $x$ and $y$ onto $C$
  (visualized via dashed lines) can only grow closer in Euclidean norm, whereas
  their projections onto $D$ (dotted lines) can spread apart.}     
...
(z - x)^\T (y - z) = 0, \quad \text{for any $y \in L$}. 
\end{equation}
as $v = y - z$ with $y, z \in L$ implies $v \in L$, and hence $-v \in L$. The 
corresponding variational equality for \smash{$z' = P_{L^\perp}(x)$} is
therefore     
\[
(z' - x)^\T (y' - z') = 0, \quad \text{for any $y' \in L^\perp$}.
\]
This holds for $z' = x - z$, as this choice leads to $-z^\T (y' - x + z) = -z^\T
y' + z^\T (x - z) = 0 + 0$, for any $y' \in L^\perp$, the first term being zero
due to $z \in L$ and $y' \in L$, and the second term being zero due to
\eqref{eq:variational_equality} with $y = 0$. This confirms
\smash{$P_{L^\perp}(x) = x - P_L(x)$} and proves  
\eqref{eq:projection_decomposition}.     
...
(x - y)^\T \big( \prox_{\lambda f}(x) - \prox_{\lambda f}(y) \big) \geq 
\| \prox_{\lambda f}(x) - \prox_{\lambda f}(y) \|_2^2, \quad \text{for all  
  $x,y$},   
\end{equation}
This is a powerful property, yet verifying it is straightforward (in a way, it
...
\[
(x - y) ^\T (z_x - z_y) = \big((x - z_x) - (y - z_y)\big)^\T (z_x - z_y) + \|z_x
- z_y\|_2^2.    
\]
...
\eqref{eq:proximal_firm_nonexpansiveness1} is equivalent to the more evocative   
property:   
\begin{equation}
...
topics from the perspective of monotone operator theory, we refer the reader to 
...
\item \label{ex:matrix_norm_proximal_mapping1}
  ...
  \prox_{\lambda f}(X) = \argmin_z \, \bigg\{ \frac{1}{2} \|X - Z\|_F^2 +
  \lambda f(Z) \bigg\},  
  \end{equation}
...
\item Assume that 
  \begin{equation}
  \label{eq:proximal_decomposition_condition}
...
\item Let \smash{$g = \sum_{i=1}^k g_i$}, with each $g_i$ closed and convex, 
  ...
  \eqref{eq:proximal_decomposition_condition} is implied by
  \begin{equation}
...
\item Show that the sufficient condition in
  \eqref{eq:proximal_decomposition_condition}, from Exercise
  \ref{ex:proximal_general_sum} part b, is equivalent to  
  \[
  \prox_f(K_i) \subseteq K_i, \quad i=1,\dots,k,
  \]
...
\item Denote by \smash{$|x|_{\TV} = \sum_{(i,j) \in E} |x_i - x_j|$}, a
  generalized TV seminorm, defined with respect to an edge set $E$ (note that
...
\item \label{ex:projection_firm_nonexpansiveness}
  ...
  \eqref{eq:variational_inequality}, note that (with $p_x$ in place of $z$ and
  $p_y$ in place of $y$):
  \[
  (p_x - x)^\T (p_y - p_x) \geq 0.
  \]
  Similarly, reversing the roles of $x$ and $y$,
  \[
  (p_y - y)^\T (p_x - p_y) \geq 0.
  \]

\begin{enumerate}[label=\alph*.]
\item By adding the above two inequalities together and rearranging, show that 
  \[
  \big( x - p_x - (y - p_y) \big)^\T (p_x - p_y) \geq 0, \quad \text{for all
    $x,y$}. 
  \]
  Hint: add the two inequalities together and rearrange.
...
\item Show that the condition from part a is equivalent to firm nonexpansiveness 
  ...
  to   
  \[
    (x - y)^\T (r_x - r_y) \geq \|r_x - r_y\|_2^2, \quad \text{for all $x,y$},
  \]
...
\item Show finally that firm nonexpansiveness is equivalent to  
  \[
...
\item In this exercise, we study a \emph{contractive} property of proximal
  ...
  \| P_C(x) - y\|_2 < \|x - y\|_2, \quad \text{for all $x \notin C$ and $y \in
    C$}.    
  \]
  Hint: recall the formulation in \eqref{eq:proximal_firm_nonexpansiveness2} of 
...
\item Assume that $f \infconv h = g \infconv h$ for a closed, convex, and
  ...
  \[
  \inf_x \, \Big\{ f_\lambda(x) - v^\T x \Big\} = \inf_x \, \Big\{ g_\lambda(x)
  - v^\T x \Big\}, \quad \text{for all $v$}.
  \]
...
\item \emph{Firm nonexpansiveness.} Prove that \smash{$\prox_f^H$} is firmly
  ...
  \[
  (x - y)^\T \big( \prox_f^H(x) - \prox_f^H(y) \big) \geq 
