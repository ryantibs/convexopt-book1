...
A formal treatment of convex optimization, as we will pursue in this book, rests 
on two key building blocks: convex sets and convex functions.

\section{Convex sets}
\label{sec:convex_sets}

A set $C \subseteq \R^d$ is called \emph{convex} if it satisfies   
\index{convex set} 
\begin{equation}
\label{eq:convex_set}
...
\emph{convex combination} of points $x_1,\dots,x_n \in \R^d$ is one of the form    
\index{convex combination} 
\[
\sum_{i=1}^n t_i x_i, \quad \text{where $t_i \geq 0$, for $i=1,\dots,n$ and 
  $\sum_{i=1}^n t_i = 1$}. 
\] 
...
\conv(C) = \bigg\{
\sum_{i=1}^n t_i x_i : 
\text{$n \geq 1$, $x_i \in C$, $t_i \geq 0$, for $i=1,\dots,n$, and 
...
\item A \emph{polyhedron} $\{x: a_i^\T x \leq b_i, \, i=1,\dots,m\}$. This is
...
Another interesting convex cone is the \emph{normal cone} to a set $C$ at a
point $x \in C$,   
\begin{equation}
\label{eq:normal_cone}
N_C(x) = \{ g : g^\T x \geq g^\T y, \, \text{for all $y \in C$}\}.
\end{equation}
For any set $C$ (convex or not), the associated normal cone $N_C(x)$ at 
...
% Separating: for example, Chapter 2.5.1 of Boyd and Vandenberghe (2004)  
% Supporting: for example, Chapter 2.5.2 of Boyd and Vandenberghe (2004) 
...
(such as \emph{Farkas' lemma}); see Exercise \ref{ex:farkas_lemma}. As another 
...
\item The exponential function $e^x$ is convex on $\R$.

\item The logarithm function $\log(x)$ is concave on $\R_{++}$.  

\index{negative entropy function}
\item The negative entropy function $x \log(x)$ is convex on $\R_{++}$. 

\item The positive part $x_+ = \max\{x,0\}$ is convex on $\R$. 

\item A linear function $a^\T x + b$ is both convex and concave on $\R^d$;
  it is the only such function with this property. 
...
f \big(t x + (1-t) y \big) \leq t f(x) + (1-t) f(y),  \quad \text{for all $x,y
  \in \R^d$ and $t \in [0,1]$}.
\]
Note that we have omitted the restriction $x,y \in \dom(f)$ from the above
statement (possible because we never encounter the undefined expressions $\infty
- \infty$ or $-\infty + \infty$, as $f$ cannot take the value $-\infty$). This
alternative view, based on well-defined arithmetic within the extended real
numbers, can be more fluid when working with functions that can take infinite
values.
\end{Remark}
...
differentiable function $f$ is strictly convex if and only if $\dom(f)$ is
convex and  
\[
...
that $f$ must be ``curved upward'' at any point $x$, in any direction $v \in
\R^d$; more precisely, letting $g(t)=f(x+tv)$, note that positive
semidefiniteness of $\nabla^2 f(x)$ implies $g''(0) = v^\T \nabla^2 f(x) v \geq
0$. For twice differentiable $f$, if $\dom(f)$ is convex and the Hessian is
positive definite everywhere, 
\[
\nabla^2 f(x) \succ 0, \quad \text{for all $x \in \dom(f)$},
\]
then $f$ is strictly convex. However, note that this is \emph{not} a necessary
condition for strict convexity: for example, $f(x) = x^4$ is strictly convex
but $f''(0)=0$.   
...
\item \parlab{xa:log_determinant}
  The \emph{log-det function}, defined by $f(X) = \log(\det(X))$, is concave on
  $\SS_{++}^d = \{X \in \SS^d : X \succ 0\}$ (the space of positive definite $d
  \times d$ matrices).  
...
If $f_1,\dots,f_n$ are convex functions and $a_1,\dots,a_n \geq 0$ then the
...
partial supremum \smash{$F = \sup_{z \in Z} \, f(\cdot,z)$}, the function
defined as  
...
$f_i$, $i=1,\dots,k$ are convex, then their pointwise maximum $F$, defined as
\smash{$F(x) = \max_{i=1,\dots,k} \, f_i(x)$}, is convex.      
...
$f$ is convex and, for each $i=1,\dots,k$, either of the following holds (where
...
\begin{enumerate}[label=\alph*.]
\item For $A_1,\dots,A_d,B \in \SS^n$, the set $\{x : x_1 A_1 + x_2 A_2 +
...
cannot grow faster than a linear function with slope $L$. Hence for real-valued
$f$, when $\nabla f$ is Lipschitz continuous ($f$ is Lipschitz smooth), one
might imagine that $f$ cannot grow faster than a quadratic function. The next 
...
\item $\|\nabla f(x) - \nabla f(y)\|_2 \leq L \|x-y\|_2$, for all $x,y \in
  \dom(f)$; \vphantom{$\frac{L}{2}$}
\item the function $f_L$ is convex, where $f_L(x) = \frac{L}{2} \|x\|_2^2 -
  f(x)$;     
\item $f(y) \leq f(x) + \nabla f(x)^\T (y-x) + \frac{L}{2} \|y-x\|_2^2$, for all
  $x,y \in \dom(f)$;
\item $(\nabla f(x) - \nabla f(y))^\T (x - y) \leq L\|x-y\|_2^2$, for all $x,y
  \in \dom(f)$. \vphantom{$\frac{L}{2}$}
\end{enumerate}
Then the following relations hold: 
\[
\text{(i)} \implies \text{(ii)} \iff \text{(iii)} \iff \text{(iv)}. 
\]
If $f$ is twice differentiable, then (ii)--(iv) are also equivalent to the
statement:  
\begin{enumerate}
\item[(v)] $\nabla^2 f(x) \preceq LI$, for all $x \in \dom(f)$.
\end{enumerate} 
Lastly, for convex $f$, statements (i)--(iv) are all equivalent (and again (v)
is equivalent if $f$ is additionally twice differentiable).
\end{Theorem}
...
\ref{ex:lipschitz_smoothness}.)  In fact, as we will learn later in Chapter 
\ref{sec:conjugate_smoothness}, there is a deeper dual relation at play:
Lipschitz smoothness of a function is formally tied to strong convexity of its 
conjugate.  
...
not differentiable at $x=0$. Meanwhile, if we stick to the univariate case, then
it seems intuitively that for a convex function the only possibilities for
...
convex functions is seomwhat remarkable, we should be careful to not
overinterpret the implications of Theorem \ref{thm:smoothness_properties}, with
respect to convex optimization. Generally speaking, sets of Lebesgue 
measure zero cannot be ignored in mathematical optimization (unlike, say,
functional analysis), particularly because many optimization problems of
interest lack smoothness at their minima; when this happens, as we will see in
the subsequent chapters, we must account for it---both analytically and 
algorithmically---and subgradients will be one of our main tools for doing so. 

\section{Cones and polyhedra*}
\label{sec:cones_polyhedra}

We cover cones and polyhedra, which play a central role in convex optimization,
in more detail. 
...
\cone(C) = \bigg\{
\sum_{i=1}^n t_i x_i : 
\text{$n \geq 1$, $x_i \in C$, $t_i \geq 0$, for $i=1,\dots,n$} \bigg\}. 
\]
...
% For example, Theorem 17.1 of Rockafellar (1970)
...
P = \{x : Ax \leq b\}
\] 
for a matrix $A$ and vector $b$, where recall that we interpret the inequality 
coordinatewise. It should be clear that $\{x : Ax \geq b\}$, $\{x : \ell \leq Ax
\leq u\}$, and $\{x : Ax \leq b, \, Gx = h\}$ are each also polyhedra, as we 
can rewrite each in the form of the above display, for appropriately (re)defined
$A,b$. 
...
x \geq 0 \}$. We will revisit this fact when we discuss linear programs in
standard form, in Chapter \ref{sec:linear_programs}. 
...
\begin{Theorem}
\label{thm:hv_representation}
Every polytope can be represented as the convex hull of a finite set of points. 
\end{Theorem}

% For example, Theorem 3 in Chapter 3.1 of Grunbaum (2003)

This result is both highly intuitive and highly nontrivial. Later, in Chapter
\ref{sec:dual_polyhedra}, we will return to it from the perspective of
duality. We can introduce some more nomenclature to help remember Theorem    
\ref{thm:hv_representation}: we call $P = \{x : Ax \leq b\}$ a \emph{halfspace
  representation} or \emph{H-representation} of $P$, and $Q =
\conv\{x_1,\dots,x_n\}$ a \emph{vertex representation} or
...
\index{Straszewicz's theorem}   
\[
C = \closure\big(\conv\{x : \text{$x$ is an exposed point of $C$}\}\big),  
\]
% For example, Theorem 8 in Chapter 2.4 of Grunbaum (2003) 
...
% For example, Theorems 1 and 5 in Chapter 2.6 of Grunbaum (2003)  
Exercise \ref{ex:face_structure} explores this in some detail. In Chapter
\ref{sec:dual_polyhedra}, we will learn about the relationship between faces of
$P$ and faces of a certain dual object $P^*$, when we interpret Theorem
\ref{thm:hv_representation} using polyhedral duality.   
...
convex function does not allow it to take the value $-\infty$ (and similarly, we
do not allow a concave function to take the value $\infty$). This differs
slightly from a standard approach in convex analysis, which allows a convex
...
arise in various settings, and there are improper convex functions which are not
trivially identical to $\pm \infty$, for example
\[
f(x) = \begin{cases}
-\infty & \text{if $\|x\| < 1$} \\
0 & \text{if $\|x\| = 1$} \\
...
simpler to rule them out completely. In short, \emph{all convex functions
  considered in this book (with nonempty effective domains) are
  proper}. Accordingly, we must occasionally add  explicit qualifiers to
...
\item \emph{Logistic regression loss.} Prove that \smash{$f(\beta) = -y^\T 
  ...
  \R^{n \times d}$ and $y \in \{0,1\}^n$, where $x_i \in \R^d$, $i=1,\dots,n$  
...
\item \emph{Hinge classification loss.} Prove that \smash{$f(\beta) =
  ...
  0\}$, for any $x_i \in \R^d$, $i=1,\dots,n$ and $y \in \{-1,1\}^n$, 
...
\item \emph{Kullback-Leibler divergence between discrete distributions.} Prove
  ...
  \]
  is a convex function of $(p,q) \in P \times P$, where $P = \{ x : \one^\T x =
  1, \, x > 0\}$.
...
\item \emph{Gaussian log likelihood for precision matrix.} Prove that 
  ...
  \R^d$, $i=1,\dots,d$ and $\mu \in \R^d$. Hint: use the relationship between 
  $\det(A)$ and $\det(A^{-1})$ for an invertible matrix $A$, and linearity of
  the trace operator $\tr(\cdot)$.  
...
\item \emph{$\ell_0$ ``norm''.}  Prove that $f(x) = \|x\|_0$, where
  ...
  positive homogeneity) and calling it a \emph{pseudonorm} is technically more
  appropriate; however, we adopt common terminology henceforth and simply
...
\item \emph{Least squares for two-layer neural network.} Prove that 
  ...
  \]
  is nonconvex over $\R^{k \times p} \times \R^k \times \R^k$, where we can take
  (for simplicity) $\phi : \R \to \R$ to be twice differentiable and monotone.    
\end{enumerate}

\item \label{ex:farkas_lemma} 
  ...
  \]  
  % For example, see proof in Chapter 2.5.1 of Boyd and Vandenberghe (2004) 
...
\index{Lipschitz smoothness} 
\item For twice differentiable $f$, show $\text{(ii)} \iff \text{(v)}$
  using the second-order characterization for convexity
  \eqref{eq:second_order_characterization}.   
  % show that $\text{(v)} \implies \text{(iii)}$ using a first-order Taylor
  % expansion of $f$ (with remainder); then show that 
  % $\text{(iv)} \implies \text{(v)}$ by expressing $\nabla^2 f(x) h$ as the
  % directional derivative of $\nabla f(x)$ in the direction $h$, applying (iv),
  % and taking $h$ to be the top eigenvector of $\nabla^2 f(x)$.  
  This establishes the second part of Theorem \ref{thm:lipschitz_smoothness}.  
...
\item For twice differentiable $f$, show $\text{(ii)} \iff \text{(v)}$
  ...
  % expansion of $f$ (with remainder); then show that 
  % $\text{(iv)} \implies \text{(v)}$ by expressing $\nabla^2 f(x) h$ as the
  % directional derivative of $\nabla f(x)$ in the direction $h$, applying
  % (iv), and taking $h$ to be the bottom eigenvector of $\nabla^2 f(x)$. 
  This establishes the second part of Theorem \ref{thm:strong_convexity}. 

\item For convex $f$, prove the final part of Theorem
  \ref{thm:lipschitz_smoothness} by showing that $\text{(iv)} \implies
  \text{(i)}$, using the following steps. Fix $x,y$ and define $F_x(z) = f(z) -
  \nabla f(x)^\T z$. Argue that $F_x$ satisfies (iv), and thus also satisfies
  (iii), that is,     
  \[
  F_x(z) \leq F_x(y) + \nabla F_x(y)^\T (z-y) + \frac{L}{2}\|z-y\|_2^2. 
  \]
  Minimize each side of the above display over $z$. (Hint: use the fact that a 
  differentiable convex function can be minimized by setting its gradient to
  zero, applied on each of the left- and right-hand sides separately.) 
...
\item First, show that the conditions $L>0$, $m>0$ in the theorems are
  not necessary, in the sense that the stated results still hold when $L=0$ and  
  $m=0$. Hint: what is a Lipschitz smooth function with $L=0$? What is a 
  strongly convex function with $m=0$?

\item Next, show that each of the latter four statements, (ii)--(v), in Theorem  
...
interpret this result, in light of parts b, c, d, as saying that for any compact
convex set $C$, the set of exposed points $\mathrm{exp}(C)$ is dense in the set
of extreme points $\mathrm{ext}(C)$. For polytopes, these two sets are finite,
and thus must be equal, which gives us another way of understanding Theorem  
\ref{thm:hv_representation}.  
...
\item Let $f$ be a convex function and $P \subseteq \dom(f)$ a polytope
  ...
  know exists from Theorem \ref{thm:hv_representation} (that is, express $P$ as 
...
\item \label{ex:convex_maximization}
  ...
  over $C$ is attained by one of the extreme points of $C$. Hint: use the
  Krein-Milman theorem, in Exercise \ref{ex:extreme_points} part b. 

\item \label{ex:face_structure} 
  Let $P \subseteq \R^d$ be a polyhedron. This exercise explores the special
  relationship between faces of $P$. Write
  \[
  P = \bigcap_{i=1}^m H_i,
  \]
  where each $H_i \subseteq \R^d$ is a halfspace, whose boundary we denote by 
  $M_i = \boundary(H_i)$ (a hyperplane), for $i=1,\dots,m$. 
% Following Theorems 2-6 in Chapter 2.6 of Grunbaum (2003)  

\begin{enumerate}[label=\alph*.]
\item Prove that each $F_i = M_i \cap P$ is a facet of $P$ (recall, this is a
  face of dimension $d-1$), for $i=1,\dots,m$. Hint: it will be convenient to
  assume, without a loss of generality, that the given H-representation for $P$
  is \emph{irredundant}: this means removing any of the halfspaces from the
  intersection gives rise to a different polyhedron.  

\item Prove that $\boundary(P) = \cup_{i=1}^m F_i$, where $F_i$, $i=1,\dots,m$
  are the facets from part a.

\item Prove that each facet $F_i$, $i=1,\dots,m$ is itself a polyhedron. 

\item Prove that each facet of a facet of $P$ is the intersection of two facets
  of $P$. 

\item Prove that each face of $P$ is the intersection of some number of facets
  of $P$. 

\item Conclude that $P$ has a finite number of faces. 
\end{enumerate}
\end{enumerate}
\end{xcb}