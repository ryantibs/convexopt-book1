...
\item Assume that problem \eqref{eq:feasibility_problem2} is feasible. Prove
  ...
  contrapositive. Note that nothing about this direction of the argument
  actually requires Slater's condition.       
...
\item In what remains, we seek to show that there exists $\beta \in \R^k$ such
  ...
  \begin{align*}
  P &= \Big\{ (x, z) \in \R^d \times \R : Ax \leq b, \, z < 0 \Big\}, \\   
  Q &= \Big\{ (x, z) \in \R^d \times \R : F(x) \leq z \Big\}. 
  \end{align*}
  ...
  \]
  Hint: apply the separating hyperplane theorem again, this time to $P,Q$.
...
\item Argue further that $\eta > 0$, so that by defining $\beta = w / z$, we
  have  
  \[
  \sup \, \Big\{ \beta^\T x : Ax \leq b \Big\} \leq \inf \, \Big\{ \beta^\T x +
  F(x) : x \in \R^d \Big\}. 
  \]
