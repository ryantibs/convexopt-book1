...
\end{equation}
Here $s$ defines a linear map which passes through $f$ at $x$ and lies below
$f$ everywhere (analogous to the first-order characterization
\eqref{eq:first_order_characterization}, where $s$ replaces $\nabla f(x)$). See
Figure \ref{fig:subgradient} for an illustration.   

\begin{figure}[tb]
\centering
\includegraphics[width=0.7\textwidth]{fig/subgradient.pdf}
\caption{Example subgradients $s_1$ and $s_2$ of a function $f$ at points $x_1$
  and $x_2$, respectively. Each $s_i$ defines a linear map that passes through
  $x_i$ and lies below $f$ everywhere (it defines a supporting hyperplane to
  $\epi(f)$ at $x_i$ with normal $(s_i,-1)$). At $x_2$, $f$ is differentiable,
  and thus $s_2 = \nabla f(x_2)$ is the only subgradient; at $x_1$ there are
  many subgradients (illustrated the gray wedge).}    
...
% Note: a simpler direct proof is possible if f is subdifferentiable on all of
% dom(f): this is equivalent to convexity (without requiring closedness of f). 
% Furthermore, strict inequality in the subgradient definition is equivalent to
% strict convexity. See Exercise \ref{ex:strict_convexity_subgradient}.
...
with respect to differentiability? The answer is nothing, in general (Exercise
...
Let $f$ be convex, and let $x \in \dom(f)$. If $f$ is differentiable at $x$,
then it has a unique subgradient $s = \nabla f(x)$ at $x$. Conversely, if $f$
has a unique subgradient $s$ at $x$, then it is differentiable at $x$ and
$\nabla f(x) = s$.     
\end{Theorem} 
...
of them. Further, if $\relint(\dom(F)) \not= \emptyset$, $Z$ is compact, $f$ is
continuous on $\relint(\dom(F)) \times Z$, and $f(\cdot,z)$ is closed and convex
...
\item For $f(x) = h_C(X)$, the support function \eqref{eq:support_function} of a 
  ...
  \[
  \partial h_C(x) = \bigg\{ y : y^\T x = \sup_{z \in C} \, x^\T z \bigg\},
  \]
  Every norm $\|\cdot\|$ is a support function which corresponds to the set  
  $C = \{x : \|x\|_* \leq 1\}$, with $\|\cdot\|_*$ being another norm that we
  call the dual norm of $\|\cdot\|$. Thus $C$ can be called the dual norm unit
  ball. The connection between $\|\cdot\|$ and $\|\cdot\|_*$ will be detailed
  later, in Chapter \ref{sec:dual_norms}, when we cover duality; for now we
  simply note that we can write   
  \index{dual norm}  
  \index{norm!ball}
  \begin{equation}
  \label{eq:norm_dual}
...
\index{normal cone}
\[
\partial I_C(x) = N_C(x) = \{ s : s^\T x \geq s^\T y, \, \text{for all $y \in
  C$}\}. 
\]
...
One way to pose a converse question precisely is as follows: given a set-valued  
...
let $(x_i, s_i)$, $i=1,\dots,n$ denote any $n$ points which lie in the graph of
$\partial f$; that is, satisfying $s_i \in \partial f(x_i)$,
$i=1,\dots,n$. Writing $x_{n+1}=x_1$ for convenience, adding together the $n$
...
The key insight in what we call \emph{Rockafellar's embedding theorem}, given
next, is that a converse statement is true as well: cyclical monotonicity
(together with maximality) is enough to ensure that a given operator is the
...
% Theorems 1 and 2 in Rockafellar (1966)
...
as well. With subgradients in place of gradients, the next result generalizes
...
\[
\partial f(x) \subseteq N_{\{y : f(y) \leq f(x)\}}(x).
\]
...
\[
N_{\{y : f(y) \leq f(x)\}}(x) = \cone\big(\partial f(x)\big).
\]
\end{Theorem}
...
analysis, we can view Theorem \ref{thm:subgradient_normal} as a generalization
of the result that the gradient vector $\nabla f$ lies orthogonal to tangent
planes of level sets a smooth function $f$. See Figure
...
\end{itemize}
Points in category-1 can be interpreted as the ``most smooth'', whereas points
in category-$d$ are ``least smooth'' (among categories 1 through $d$, excluding 
...
union of points in categories 2 through $d$ only form a set of Lebesgue measure
...
The proof of Theorem \ref{thm:subgradient_category_d} is elementary and 
rests critically on the normal cone representation of subgradients in Theorem 
\ref{thm:subgradient_normal}. It uses the normal cone relationship (and
convexity of the sublevel set) to argue that the interiors of conic hulls of
category-$d$ subdifferentials for points in a level set form a collection of 
disjoint open sets in $\R^d$, thus by classical arguments in analysis, there
can be at most a countable number of them. See Exercise
...
thesis \cite{rockafellar1963convex} and \cite{moreau1963inf,
  moreau1963proprietes}. A classic reference on subdifferential theory (and a 
masterful treatment) is \cite{rockafellar1970convex} (Chapters 23--25). We also
recommend \cite{hiriartUrruty2001fundamentals} (Chapter D) and
\cite{bertsekas2009convex} (Chapter 5.4). The study of subgradients is closely
related to the study of monotone operators (and also to that of proximal
operators, which will be covered in the next chapter). Two nice books that
develop this connection from different perspectives (analytic versus
...
\item \label{ex:converse_supporting_hyperplane}
  ...
  ($\aff(C) = \R^d$), and so its relative interior and relative boundary are its
  interior and boundary, respectively. (Note: having proved such a
  ...
  result just proved to conclude that this set, say, $D$ (the reparametrization 
  of $C$ to the affine subspace) is convex in this coordinate system. Then we 
  simply view $C$ as an affine image of $D$ in order to conclude that $C$ itself
  is convex.)  
...
\item \label{ex:operator_norm_subgradients}
  ...
  \|X\|_{\op} = \sup_{\|Z\|_{\tr} \leq 1} \, \langle X, Z \rangle. 
  \end{equation}
...
\item \label{ex:trace_norm_subgradients} 
  ...
  \|X\|_{\tr} = \sup_{\|Z\|_{\op} \leq 1} \, \langle X, Z \rangle.
  \end{equation}
...
constant. The arguments used to prove this part are more advanced. What can be 
shown ``easily'': for closed convex $f$, we have $\partial f \subseteq \partial
g$ for some convex closed $g$, with $\partial g$ maximal cyclically
...
\item Now we will work toward $\text{(iv)} \implies \text{(ii)}$, in steps laid
...
\item \label{ex:strict_convexity_subgradient} 
  Prove that a strictly convex function must yield a strict inequality in
  \eqref{eq:subgradient} when $y \not= x$. And conversely, prove that if $f$
  is finite and subdifferentiable on a convex set $C$, and satisfies
  \eqref{eq:subgradient} on $C$ with strict inequality for $y \not =x$, then $f$
